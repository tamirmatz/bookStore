...
var gBooks = storageService.loadFromStorage(KEY) ? storageService.loadFromStorage(KEY) : [{
        "id": "OXeMG8wNskc",
...
function getBooksFromApi(filter) {
    const APIKEY = 'AIzaSyCLkoIgUJN1yOXCxGRNdy-nWLHKHj4XsL4'
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${filter}:keyes&key=${APIKEY}`)
...
function deleteReview(reviewId, bookId) {
    getBookById(bookId)
        .then(book => {
            const bookReviews = book.bookReviews
            const reviewIdx = bookReviews.findIndex(review => {
...
function query(filterBy = {}) {
    if (filterBy) {
        ...
        const filteredBooks = gBooks.filter(book => {
            ...
            return book.title.includes(title) && amount >= minPrice && amount <= maxPrice
        })
...
function _ajustBook(book) {
    const authors = !book.volumeInfo.authors ? ['SomeWriter'] : book.volumeInfo.authors
    const categories = !book.volumeInfo.categories ? ['General'] : book.volumeInfo.categories
    const ajustedBook = {
        "id": utilService.makeId(),
...
function addBook(book) {
    const adustedBook = _ajustBook(book)
    gBooks.push(adustedBook)
    storageService.saveToStorage(KEY, gBooks)
}
